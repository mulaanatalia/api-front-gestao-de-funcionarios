...
class FuncionarioService():
    ...
    def get_all_funcionarios(self, filtros):
        ...
        try:
            ...
            p.nome as provincia, d.nome as distrito, dp.nome as departamento, f.contacto
                    ...
                    INNER JOIN departamento dp on dp.id = f.id_departamento where {sql_filtros} and estado = 1 order by f.nome asc"""
    ...
    def post_funcionario(self,funcionario):
        try:
            ...
            (nome,genero,data_nascimento,contacto,id_provincia,id_distrito,id_departamento,caminho)
    ...
    def get_one_funcionarios(self, id):
        try:
            query = """SELECT f.id, f.nome , f.genero as genero, f.data_nascimento, f.caminho,
            p.nome as provincia, d.nome as distrito, dp.nome as departamento, f.contacto
                    FROM funcionario f
                    INNER JOIN provincia p on p.id = f.id_provincia
                    INNER JOIN distrito d on d.id = f.id_distrito
